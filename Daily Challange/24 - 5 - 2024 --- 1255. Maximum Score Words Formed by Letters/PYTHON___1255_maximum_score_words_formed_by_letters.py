...
class Solution:
      def maxScoreWords(self, words: List[str], letters: List[str], score: List[int]) -> int:
            # This will make a hashmap which stores the letters and their freq in letters list
            letter_count = Counter(letters)

            def can_form_word(word, letter_count):
                  # make a hashmap which will store the count of each word available in our words[i]
                  word_count = Counter(word)

                  # we will check for every char count from our letter_count ... does we have enough letters available ?

                  for char in word_count:
                  # word_count[char] --> check the no of time one char comes in words[i]
