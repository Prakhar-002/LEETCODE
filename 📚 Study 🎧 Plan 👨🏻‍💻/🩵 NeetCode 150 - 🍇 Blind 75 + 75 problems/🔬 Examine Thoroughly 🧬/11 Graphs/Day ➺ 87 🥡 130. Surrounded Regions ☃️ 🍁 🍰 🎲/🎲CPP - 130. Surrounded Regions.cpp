...
public:
      void solve(vector<vector<char>>& board) {
            int ROWS = board.size(), COLS = board[0].size(); // Dimensions of the board

            // Step 1: Run DFS for 'O's on the left and right borders
            for (int r = 0; r < ROWS; r++) {
                  if (board[r][0] == 'O') capture(r, 0, board, ROWS, COLS);
                  if (board[r][COLS - 1] == 'O') capture(r, COLS - 1, board, ROWS, COLS);
            }

            // Step 2: Run DFS for 'O's on the top and bottom borders
            for (int c = 0; c < COLS; c++) {
                  if (board[0][c] == 'O') capture(0, c, board, ROWS, COLS);
                  if (board[ROWS - 1][c] == 'O') capture(ROWS - 1, c, board, ROWS, COLS);
            }
...
private:
      void capture(int r, int c, vector<vector<char>>& board, int ROWS, int COLS) {
