...
#include <stdio.h>
      ...
      // and count the soldier that are right 
      // and have greater rating than the current soldier

      // now calculate the descending order

      // we count the soldier that have greater rating and left of our current soldier
      // and count the soldier that are right 
      // and have less rating than the current soldier

      // then we will multiply them and add to our team 
      // cause that will be the number of possible teams
...
int numTeams(int *rating, int ratingSize) {
      ...
      for (int soldier = 1; soldier < ratingSize - 1; soldier++) {
            ...
            for (int s = soldier + 1; s < ratingSize; s++) {
                  if (rating[s] > rating[soldier]) {
                        rightLarger += 1;
                  }
            }
            ...
            int rightSmaller = ratingSize - soldier - 1 - rightLarger;
