...
class Solution:
      def minSteps(self, n: int) -> int:
            # initialized with a length of `n + 1` and every element is set to max possible
            keyBoard = [1000] * (n + 1)
            ...
            for num in range(2, n + 1):
                  ...
                  for factor in range(1, (num // 2) + 1):
                        ...
                        if num % factor == 0:
                              # algorithm calculates the possible minimum steps as `keyBoard[factor] + num // factor` 
                              keyBoard[num] = min( 
                                    # updates the `keyBoard[num]` with the lesser value between the current `keyBoard[num]` and newly calculated steps
                                    keyBoard[num],
                                    # `keyBoard[factor]` gives the steps needed to first get `factor` 'A's
                                    # we add `num // factor`, which is the number of times we need to paste `factor` 'A's to get `num` 'A's
...
class Solution:
      # `min_steps` method essentially calculates the sum of prime factors of the given number `n`
      def minSteps(self, n: int) -> int:
            operations = 0
            i = 2
            while i * i <= n:
