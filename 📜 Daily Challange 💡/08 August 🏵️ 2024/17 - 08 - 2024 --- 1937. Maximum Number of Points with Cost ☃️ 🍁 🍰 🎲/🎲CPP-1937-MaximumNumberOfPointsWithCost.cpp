...
public:
      long long maxPoints(vector<vector<int>>& points) {
            ...
            vector<long long> prevRowPoints(COLS);
            ...
            for (int row = 1; row < ROWS; row++) {
                  ...
                  vector<long long> nextRowPoints(COLS);

                  // maximum points that can be obtained from the left side and right side
                  vector<long long> leftMaxSum(COLS);
                  vector<long long> rightMaxSum(COLS);
                  ...
                  for (int col = 0; col < COLS; col++) {
                        ...
                        nextRowPoints[col] = points[row][col] + max(leftMaxSum[col], rightMaxSum[col]);
                  }
            ...
            long long maxPoints = prevRowPoints[0];
            for (int i = 1; i < COLS; i++) {
                  maxPoints = max(maxPoints, prevRowPoints[i]);
            }
