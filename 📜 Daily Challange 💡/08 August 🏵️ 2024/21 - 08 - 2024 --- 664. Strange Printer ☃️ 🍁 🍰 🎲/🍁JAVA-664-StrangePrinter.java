...
class Solution {
      ...
      private int util(int i, int j, String s, int[][] dp) {
            ...
            // checks if the solution for the substring `s.substring(i, j+1)` has already
            // been computed
            if (dp[i][j] != -1) {
            ...
            for (int k = i + 1; k <= j; k++) {
                  ...
                  if (s.charAt(k) == firstLetter) {
                        // Computes the total prints needed if the string is split into `s.substring(i,
                        // k)` and `s.substring(k+1, j+1)`
                        // It combines the results of these two parts, as `s.charAt(k)` matches
                        // `s.charAt(i)` and thus doesn't require an extra print
                        int betterPrints = util(i, k - 1, s, dp) + util(k + 1, j, s, dp);
