...
class Solution {
      public List<Integer> lexicalOrder(int n) {
            ...
            List<Integer> lexicographical = new ArrayList<>();

            // used to generate the next number in lexicographical order
            int cur = 1;

            // until the size of the `lexicographical` list is less than `n`
            // ensures that exactly `n` numbers are added to the list
            while (lexicographical.size() < n) {
                  // current value of `cur` (starting with 1) is added to the list
                  lexicographical.add(cur);
                  ...
                  if (cur * 10 <= n) {
                        ...
                        // `cur` is incremented by 1. This is akin to moving to the next sibling in a
                        // tree structure
                        // (e.g., from 19 to 20).
                        cur++;
                  }
