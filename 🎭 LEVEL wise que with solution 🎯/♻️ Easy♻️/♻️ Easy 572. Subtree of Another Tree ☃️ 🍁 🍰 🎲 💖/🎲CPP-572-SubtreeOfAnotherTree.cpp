...
public:
      bool isSubtree(TreeNode* root, TreeNode* subRoot) {
            ...
            if (!subRoot) {
                  // If `subRoot` is `nullptr`, returns `true`
                  return true;
            }

            // An empty tree cannot contain a non-empty subtree
            if (!root) {
                  // If `root` is `nullptr` but `subRoot` is not, returns `false`
                  return false;
            }
...
private:
      bool isSameTree(TreeNode* root, TreeNode* subRoot) {
            // If both `root` and `subRoot` are `nullptr`
            if (!root && !subRoot) {
                  // returns `true`
                  return true;
            }

            // If both `root` and `subRoot` are not `nullptr` and their values (`val`) are the same
